...
WorldMap.prototype.createMap = function(){
	...
	if(typeof jQuery != 'undefined') {
		...
		jQuery('body').append('<div id="'+this.id+'_framerate" style="position:absolute;padding:0px;bottom:5px;right:5px;z-index:20;font-size:12px;">Framerate</div>');
	}
...
WorldMap.prototype.draw = function() {
	if(this.canvas && this.canvas.getContext){
		...
		this.ctx.restore();
		jQuery("#"+this.id+"_framerate").html((new Date() - this.clock) + "ms");
