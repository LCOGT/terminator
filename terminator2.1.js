...
console.log('This height '+this.height)
			//if(typeof jQuery != 'undefined') $('#'+this.id).css('height',this.height);
		}else{
...
console.log(this.height)
	...
	for(var i = 0; i < this.placemarks.length ; i++){
		...
		jQuery('body').append('<div class="worldmap_label" style="position:absolute;top:'+(off.top+p.y-(this.fs/2))+'px;left:'+(off.left+p.x-this.d/2)+'px;width:'+w+'px;height:'+(this.fs)+'px;display:block;font-size:'+this.fs+'px;'+((p.link) ? '' : 'padding:0px 0px 0px '+(this.d*1.5)+'px;')+';z-index:20;">'+label+'</div>');
	...
	try {
		...
		console.log('Could not get image data for day')
	}
	...
	try {
		...
		console.log('Could not get image data for night')
	}
	// For excanvas we need to initialise the newly created <canvas>
...
//			image2.data = image2Data;
//			this.ctx.putImageData(image2, 0, 0);
